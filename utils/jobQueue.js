...
class JobQueue {
  constructor(config = {}) {
    ...
    this.maxConcurrent =
      config.maxConcurrent || parseInt(process.env.QUEUE_WORKERS) || 3;
    this.enabled =
  ...
  async restore() {
    ...
    try {
      ...
      const pendingJobs = await this.jobStore.loadAllJobs({
        status: "pending",
      });
      ...
      const processingJobs = await this.jobStore.loadAllJobs({
        status: "processing",
      });
      ...
      if (processingJobs.length > 0) {
        logger.info(
          `Reset ${processingJobs.length} interrupted jobs to pending`,
        );
      }
  ...
  async updateJob(jobId, status, updates = {}) {
    ...
    if (this.jobStore) {
      await this.jobStore.saveJob(job).catch((err) => {
        logger.warn("Job persistence error", { error: err.message });
      });
    }
  }
   ...
   */
  async clearCompleted(olderThan = 3600000) {
    // 1 hour default
    // Clear from store
    if (this.jobStore) {
