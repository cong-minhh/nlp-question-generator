...
class DifficultyBalancer {
  constructor(config = {}) {
    this.enabled =
      config.enabled !== undefined
        ? config.enabled
        : process.env.DIFFICULTY_BALANCE_ENABLED !== "false";
    ...
    this.tolerance =
      config.tolerance ||
      parseFloat(process.env.DIFFICULTY_BALANCE_TOLERANCE) ||
      0.1; // 10% tolerance
    this.maxRetries =
      config.maxRetries ||
      parseInt(process.env.DIFFICULTY_BALANCE_MAX_RETRIES) ||
      2;
  }
  ...
  async balance(questions, regenerateFn = null, attempt = 1) {
    ...
    logger.info(
      `Balancing difficulty distribution (attempt ${attempt}/${this.maxRetries + 1})...`,
    );
    ...
    logger.info(
      `Current: Easy ${balanceCheck.current.easy}, Medium ${balanceCheck.current.medium}, Hard ${balanceCheck.current.hard}`,
    );
    logger.info(
      `Target:  Easy ${balanceCheck.target.easy}, Medium ${balanceCheck.target.medium}, Hard ${balanceCheck.target.hard}`,
    );
    ...
    logger.info(
      `⚠ Distribution imbalanced (max deviation: ${(parseFloat(balanceCheck.maxDeviation) * 100).toFixed(1)}%)`,
    );
    ...
    const toRegenerate = [];
    if (needed.easy > 0)
      toRegenerate.push({ difficulty: "easy", count: needed.easy });
    if (needed.medium > 0)
      toRegenerate.push({ difficulty: "medium", count: needed.medium });
    if (needed.hard > 0)
      toRegenerate.push({ difficulty: "hard", count: needed.hard });
    ...
    logger.info(
      `Regenerating: ${toRegenerate.map((t) => `${t.count} ${t.difficulty}`).join(", ")}`,
    );
    ...
    try {
      ...
      logger.warn("Rebalancing failed", { error: error.message });
      return {
        questions,
        balanced: false,
        distribution,
  ...
  async balanceByRemoval(questions, distribution) {
    ...
    logger.info(
      `✓ Removed ${questions.length - balanced.length} excess questions`,
    );
