...
class QualityScorer {
  constructor(config = {}) {
    this.enabled =
      config.enabled !== undefined
        ? config.enabled
        : process.env.QUALITY_SCORING_ENABLED !== "false";
    this.minScore =
      config.minScore || parseInt(process.env.QUALITY_MIN_SCORE) || 6;
    this.maxRetries =
      config.maxRetries || parseInt(process.env.QUALITY_MAX_RETRIES) || 2;
    this.batchSize = config.batchSize || 5;
    this.scorerProvider = config.scorerProvider || null; // Provider instance for scoring
    this.useQuickScore =
      config.useQuickScore !== undefined
        ? config.useQuickScore
        : process.env.QUALITY_QUICK_SCORE === "true";
  }
  ...
  async scoreQuestion(question) {
    ...
    try {
      ...
      if (response.questions && response.questions.length > 0) {
        ...
        const textToParse =
          firstQuestion.questiontext || JSON.stringify(firstQuestion);
      ...
      if (process.env.NODE_ENV === "development") {
        logger.warn("Scoring failed", { error: error.message });
      }
      return {
        score: this.minScore,
        passed: true,
  ...
  async scoreQuestions(questions) {
    ...
    for (let i = 0; i < questions.length; i += this.batchSize) {
      ...
      try {
        ...
        logger.warn(
          `Batch scoring failed for questions ${i}-${i + batch.length}`,
          { error: error.message },
        );
  ...
  async scoreBatch(questions) {
    try {
      ...
      logger.warn("Batch scoring parse error", { error: error.message });
        ...
        try {
          ...
          if (scoreMatch) {
            return {
              ...
              recommendation:
                parseInt(scoreMatch[1]) >= 7 ? "accept" : "reject",
            };
          }
        }
      }
  ...
  filterByScore(questions, scores) {
    ...
    return {
      ...
      needsRevision: needsRevision.map((n) => ({
        question: n.question,
        score: n.score,
      })),
      scores: {
  ...
  async scoreAndImprove(questions, regenerateFn = null, attempt = 1) {
    ...
    logger.info(
      `Scoring ${questions.length} questions (attempt ${attempt}/${this.maxRetries + 1})...`,
    );
    ...
    logger.info(
      `Accepted: ${filtered.accepted.length}, Rejected: ${filtered.rejected.length}, Needs revision: ${filtered.needsRevision.length}`,
    );
    ...
    if (
      ...
      logger.info(
        `Regenerating ${filtered.rejected.length} low-quality questions...`,
      );
      ...
      try {
        ...
        logger.warn("Regeneration failed", { error: error.message });
        // Return what we have
        return {
    ...
    const finalQuestions = [
      ...filtered.accepted,
      ...filtered.needsRevision.map((n) => n.question),
    ];
    const finalScores = [
      ...filtered.scores.accepted,
      ...filtered.scores.needsRevision,
    ];
