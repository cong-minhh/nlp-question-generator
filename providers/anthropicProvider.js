...
class AnthropicProvider extends BaseAIProvider {
  ...
  validateConfig() {
    if (!this.config.apiKey) {
      throw new Error(
        "Anthropic API key is required. Set ANTHROPIC_API_KEY environment variable or configure in settings."
      );
    }
    if (
      this.config.model &&
  ...
  async initialize(config = {}) {
    ...
    logger.info(`Initialized Anthropic`, { model: this.currentModel });
  }
  ...
  async tryFallbackModel() {
    ...
    if (this.currentModelIndex >= this.supportedModels.length) {
      logger.error("All Anthropic fallback models exhausted");
      return false;
    }

    const fallbackModel = this.supportedModels[this.currentModelIndex];
    logger.info(`Falling back to Anthropic model`, { model: fallbackModel });
      ...
      try {
        ...
        if (typeof text === "object" && text.text) {
          ...
          if (text.images && Array.isArray(text.images)) {
            ...
            logger.debug(`Adding images to Anthropic prompt`, {
        ...
        if (error.message && error.message.includes("404")) {
          logger.warn(`Anthropic model not available (404)`, {
            model: this.currentModel,
          });
          ...
          if (fallbackSuccess) {
            ...
            throw new Error(
              `All Anthropic models unavailable. Please check your API key or subscription tier.`
            );
          }
        }
        ...
        if (
          ...
          if (!isLastAttempt) {
            ...
            logger.warn(`Anthropic rate limited after all attempts`, {
              model: this.currentModel,
              attempts: this.maxRetries,
            });
            throw new Error(
              `Anthropic model ${this.currentModel} is rate limited. Please try again later or upgrade your plan.`
            );
          }
        }
        ...
        if (
          ...
          if (!isLastAttempt) {
            logger.warn(`Anthropic overloaded`, {
              model: this.currentModel,
              attempt,
              maxRetries: this.maxRetries,
              retryDelaySec: delay / 1000,
            ...
            logger.warn(`Anthropic overloaded after all attempts`, {
              model: this.currentModel,
              attempts: this.maxRetries,
            });
            throw new Error(
              `Anthropic model ${this.currentModel} is currently overloaded. Please try again in a few moments.`
            );
          }
        }
