...
class GeminiProvider extends BaseAIProvider {
  ...
  validateConfig() {
    if (!this.config.apiKey) {
      throw new Error(
        "Gemini API key is required. Set GEMINI_API_KEY environment variable or configure in settings."
      );
    }
    if (
      this.config.model &&
  ...
  async generateQuestions(text, options = {}) {
    ...
    const promptOptions = {
      ...
      imageMetadata: options.imageMetadata || [], // Pass image IDs so AI can reference them
      distributionPlan: options.distributionPlan || null, // For distributed question generation
    };
      ...
      try {
        ...
        if (error.message && error.message.includes("404")) {
          ...
          if (fallbackSuccess) {
            ...
            throw new Error(
              `All Gemini models unavailable. Please check your API key or try again later.`
            );
          }
        }
        ...
        if (error.message && error.message.includes("503")) {
          ...
          if (!isLastAttempt) {
            logger.warn(
              `Gemini API: Attempt ${attempt}/${this.maxRetries} - ${
                this.currentModel
              } overloaded. Retrying in ${delay / 1000}s...`
            );
            ...
            logger.warn(
              `${this.currentModel} is overloaded after ${this.maxRetries} attempts. Please try again later.`
            );
            throw new Error(
              `Gemini model ${this.currentModel} is currently overloaded. Please try again in a few moments.`
            );
          }
        }
    ...
    for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
      try {
        ...
        if (images && Array.isArray(images)) {
          ...
          logger.debug(
            `Adding ${images.length} images to Gemini generic prompt`
          );
        }
