...
class DeepSeekProvider extends BaseAIProvider {
  ...
  validateConfig() {
    if (!this.config.apiKey) {
      throw new Error(
        "DeepSeek API key is required. Set DEEPSEEK_API_KEY environment variable or configure in settings."
      );
    }
    if (
      this.config.model &&
  ...
  async initialize(config = {}) {
    ...
    logger.info(`Initialized DeepSeek`, { model: this.currentModel });
  }
  ...
  async tryFallbackModel() {
    ...
    if (this.currentModelIndex >= this.supportedModels.length) {
      logger.error("All DeepSeek fallback models exhausted");
      return false;
    }

    const fallbackModel = this.supportedModels[this.currentModelIndex];
    logger.info(`Falling back to DeepSeek model`, { model: fallbackModel });
...
Return ONLY a valid JSON object with this exact structure (no markdown, no code blocks, just raw JSON):
{
    ...
    for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
      try {
        ...
        if (error.message && error.message.includes("404")) {
          logger.warn(`DeepSeek model not available (404)`, {
            model: this.currentModel,
          });
          ...
          if (fallbackSuccess) {
            ...
            throw new Error(
              `All DeepSeek models unavailable. Please check your API key or subscription.`
            );
          }
        }
        ...
        if (
          ...
          if (!isLastAttempt) {
            logger.warn(`DeepSeek rate limited`, {
              model: this.currentModel,
              attempt,
            ...
            logger.warn(`DeepSeek rate limited after all attempts`, {
              model: this.currentModel,
              attempts: this.maxRetries,
            });
            throw new Error(
              `DeepSeek model ${this.currentModel} is rate limited. Please try again later.`
            );
          }
        }
        ...
        if (
          ...
          if (!isLastAttempt) {
            logger.warn(`DeepSeek overloaded`, {
              model: this.currentModel,
              attempt,
              maxRetries: this.maxRetries,
              retryDelaySec: delay / 1000,
            ...
            logger.warn(`DeepSeek overloaded after all attempts`, {
              model: this.currentModel,
              attempts: this.maxRetries,
            });
            throw new Error(
              `DeepSeek model ${this.currentModel} is currently overloaded. Please try again in a few moments.`
            );
          }
        }
  ...
  async testConnection() {
    try {
      ...
      const testText =
        "DeepSeek is an advanced AI model developed for various natural language processing tasks.";
  ...
  getModelInfo() {
    return {
      "deepseek-chat": {
        name: "DeepSeek Chat",
        description:
          "Optimized for conversational tasks and general question generation",
