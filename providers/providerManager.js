...
class ProviderManager {
  constructor(config = {}) {
    ...
    this.defaultProvider =
      process.env.DEFAULT_PROVIDER || config.defaultProvider || "gemini";
    ...
    logger.debug(`Default provider set to: ${this.defaultProvider}`);
    if (this.router.enabled) {
      logger.debug(`Smart routing enabled`, {
        strategy: this.router.routingStrategy,
      });
    }
  }
  ...
  async initialize(config = {}) {
    ...
    if (!this.hasProvider(this.currentProvider)) {
      logger.warn(`Current provider not available, falling back`, {
        requested: this.currentProvider,
        fallback: this.defaultProvider,
      });
  ...
  async loadProviders(config = {}) {
    ...
    try {
      ...
      logger.debug("Gemini provider loaded");
    } catch (error) {
      logger.warn("Gemini provider failed to load", { error: error.message });
    }
    ...
    try {
      ...
      logger.debug("OpenAI provider loaded");
    } catch (error) {
      logger.warn("OpenAI provider failed to load", { error: error.message });
    }
    ...
    try {
      ...
      logger.debug("Anthropic provider loaded");
    } catch (error) {
      logger.warn("Anthropic provider failed to load", {
        error: error.message,
      });
    }
    ...
    try {
      ...
      logger.debug("DeepSeek provider loaded");
    } catch (error) {
      logger.warn("DeepSeek provider failed to load", { error: error.message });
    }
    ...
    try {
      ...
      logger.debug("Kimi Global provider loaded");
    } catch (error) {
      logger.warn("Kimi Global provider failed to load", {
        error: error.message,
      });
    }
    ...
    try {
      ...
      logger.debug("Kimi CN provider loaded");
    } catch (error) {
      logger.warn("Kimi CN provider failed to load", { error: error.message });
    }
    ...
    try {
      ...
      logger.debug("Local provider loaded");
    } catch (error) {
      logger.warn("Local provider failed to load", { error: error.message });
    }
  }
  ...
  switchProvider(providerName) {
    ...
    logger.info(`Switched provider`, { provider: providerName });
  }
  ...
  async testAllProviders() {
    ...
    logger.info("Testing all available providers...");

    for (const providerInfo of availableProviders) {
      try {
        const provider = this.getProvider(providerInfo.name);
        logger.debug(`Testing provider`, { provider: providerInfo.name });
