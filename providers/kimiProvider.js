...
class KimiProvider extends BaseAIProvider {
  ...
  validateConfig() {
    if (!this.config.apiKey) {
      throw new Error(
        "Kimi API key is required. Set KIMI_API_KEY environment variable or configure in settings."
      );
    }
    if (
      this.config.model &&
  ...
  async initialize(config = {}) {
    ...
    logger.info(`Initialized Kimi`, { model: this.currentModel });
  }
  ...
  async tryFallbackModel() {
    ...
    if (this.currentModelIndex >= this.supportedModels.length) {
      logger.error("All Kimi fallback models exhausted");
      return false;
    }

    const fallbackModel = this.supportedModels[this.currentModelIndex];
    logger.info(`Falling back to Kimi model`, { model: fallbackModel });
